...
int main() {
	...
	while ((current != gameover) && (current != EXIT) && (current != win)) {
		...
		move_check(field, movable);
		if (((current != next) && (current != undo) && (current != failed_to_load)&&(current!=nothing)&&(turns_before !=turns)))setrand2(field, isempty);
		move_check(field, movable);
